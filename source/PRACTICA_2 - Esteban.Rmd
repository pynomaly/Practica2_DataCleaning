...
-   La variable `page_num` incluye `r length(unique(data$page_num))` posibilidades, que están en el rango `r range(data$page_num)`, porque en la descarga se han elegido las primeras 15 páginas de cada topic buscado. Podemos ver que tenemos un número similar de casos imágenes en cada número de página.
...
ggplot(data, aes(x = image_views)) +
  geom_boxplot(fill = "darkblue", color = "black", alpha = 0.7) +
  labs(title = "Boxplot de comentarios",
       x = "Número de Vistas",
       y = "Frecuencia") +
  theme_minimal()
```
...
En `r round(sum(data$private_collections == 0) / nrow(data) *100, 2)`% de los registros no están en ninguna colección privada. Puede ser una variable que no aporte demasiada información. Sin embargo la analizaremos más adelante a detalle.


-   La variable `image_size` es numérica, con un valor mínimo de `r min(data$image_size)`, un valor máximo de `r max(data$image_size)`, una media de `r round(mean(data$image_size))`, con esta distribución sin embargo cabe recalcar que aquellos valores pequeños corresponden a MB mientras que aquellos valores grandes son KB por ende es necesario una transformación a una sola escala para poder analizar mejor esta variable:

```{r}
ggplot(data, aes(x = image_size)) +
  geom_histogram(fill = "darkgreen", color = "black", alpha = 0.7) +
  labs(title = "Distribución de tamaño de imagen",
       x = "Tamaño de imagen en MB",
       y = "Frecuencia") +
  theme_minimal()
```
Por otro lado la variable `image_px` es una variable categórica que contiene el número de pixeles que contiene la imagen en alto y ancho. Podemos ver que tenemos un número extenso de resolucones de cada imagen sin embargo el grupo más importante es el de aquellas imágenes que tienen una resolución de 1920X1080. Esta variable guarda además una relación con la distribución de `image_size` ambas se ubican se concentran a la izquierda. 
...
Eliminanos las variables con gran número de NA (`location`). Mantener una de las dos variables de dimensiones: `size` o `superficie`, eliminar la otra si esque encontramos una correlación grande entre las dos.

-   La variable `location` tiene un alto porcentaje de valores vacíos (""), que constituyen el `r round((nrow(data[data$location == "", ]) / nrow(data)) * 100, 2)`% de los registros. Decidimos no incluir esta variable en el análisis posterior.
...
Valores ausentes de cada variable: Tenemos presencia de outliers en variables como image_license, image_px e image_size aunque no es un problema demasiado grande se puede optar por eliminar estas observaciones pues son un número reducido de casos.
...
# El siguiente código realiza un preprocesado de datos para dejarlos en un formato más útil que al inicio.
...
4.1. Aplica un modelo supervisado y uno no supervisado a los datos y comenta los resultados obtenidos. 4.2. Aplica una prueba por contraste de hipótesis. Ten en cuenta que algunas de estas pruebas requieren verificar previamente la normalidad y homocedasticidad de los datos.



### Descripción del paso de preparación de datos.
...
selected_vars = c('image_favs_cat', 'search_topic', 'page_num', 'image_com', 'image_views', 'image_size_kb', 'published_date', 'image_license',"year", "month", "day_of_week",
          "moment_of_day", "private_collections", "superficie_px2")
...
A continuacion realizamos una analisis exploratorio de las variables y su relacion con la variable objetivo. 
...
Antes de realizar esta modelización quiero que podamos analizar los datos disponibles. 
...
Al realizar los siguientes scatter plots entre las variables predictoras y la variable objetivo, observamos lo siguiente:
...
Al analizar los siguientes gráficos de densidad de las variables predictoras en relación con la variable objetivo (`image_favs_cat`), observamos lo siguiente:
...
```
## **Modeilzación utilizando random forest

Ahora que entendemos un poco que variables separan mejor nuestros datos de acuerdo al nivel de favoritos recibidos realizaremos un random forest y analizaremos el rendimiento del modelo con los datos no vistos de test. 
...
Matriz de Confusión: - **Very Low**: Clasificado correctamente 313 veces. - **Low**: Clasificado correctamente 263 veces. - **Medium**: Clasificado correctamente 258 veces. - **High**: Clasificado correctamente 283 veces.

Overall Statistics: - **Accuracy**: 0.8088 (80.88%) El modelo clasifica correctamente aproximadamente el 80.88% de las instancias.

-   **95% CI**: (0.7871, 0.8293) El intervalo de confianza del 95% para la precisión se encuentra entre 78.71% y 82.93%.

-   **No Information Rate (NIR)**: 0.2571 La tasa de no información indica la precisión que se obtendría al clasificar siempre en la clase más frecuente.

-   **P-Value [Acc \> NIR]**: \< 2.2e-16 Indica que la precisión del modelo es significativamente mayor que la del modelo sin información.

-   **Kappa**: 0.745 El índice de Kappa indica un buen acuerdo entre las predicciones del modelo y las clases reales.
...
-   **Precisión del Modelo**: La precisión del modelo es buena (81.28%), con una alta sensibilidad y especificidad en la mayoría de las clases.
...
Por otro lado hemos decidido analizar 4 clústeres aunque hemos tenido tenido resultados contrarios en el método de la silueta y del codo. Pues el primero nos indicaba un número de 2 clusters mientras que el método del codo almenos 4 clústeres. Hemos decidido ir por 4 clústeres para una mejor separación de características.
...
$H_0$: No hay asociación entre el tema de búsqueda (`search_topic`) y la categoría del número de favoritos (`image_favs_cat`). $H_1$: Existe una asociación entre el tema de búsqueda (`search_topic`) y la categoría del número de favoritos (`image_favs_cat`).
